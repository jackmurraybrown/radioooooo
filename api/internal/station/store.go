...
// ⊹ ࣪ ˖ returns station settings needed by the expansion job
func (s *Store) SettingsForChannel(ctx context.Context, channelID string) (string, int, *time.Time, error) {
	var tz string
	var horizon int
	var seasonEnd *time.Time
	err := s.db.QueryRow(ctx, `
		select st.timezone, st.default_show_horizon_days, st.season_end_date
		from stations st
		join channels c on c.station_id = st.id
		where c.id = $1::uuid
	`, channelID).Scan(&tz, &horizon, &seasonEnd)
	return tz, horizon, seasonEnd, err
}
